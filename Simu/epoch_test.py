...
for node in cluster.node_list:
  if(node.ip == '172.31.4.135'):
    node1 = node
  elif(node.ip == '172.31.3.225'):
    node2 = node

num_task = 1
for task in workload[0].task_list:
  if(num_task % 2 == 1):
    task.worker_ip = node1.ip
    task.ps_port = master.get_available_port()
    task.occupied_worker_port = node1.get_available_port()
    node1.queue.append(task)
  else:
    task.worker_ip = node2.ip
    task.ps_port = master.get_available_port()
    task.occupied_worker_port = node2.get_available_port()
    node2.queue.append(task)
  num_task += 1
