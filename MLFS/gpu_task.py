...
def main(_):
  ...
  if FLAGS.job_name == "ps":
    ...
    with tf.train.MonitoredTrainingSession(master=server.target,
      ...
      saver.save(get_session(mon_sess), train_folder+"/latest_model_"+j_name+".ckpt")
    ...
    with tf.Session(target = server.target) as sess:
      ...
      saver.restore(sess, train_folder+"/latest_model_"+j_name+".ckpt")
      cv_auc_list = []
      cv_acc_list = []
      cv_loss_list = []
...
if __name__ == "__main__":
  ...
  if(role == 'worker'):
    ...
    completion_task(job_name, task_index, worker_ip, worker_host)
